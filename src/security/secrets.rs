...
// in `~/.hrafn/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.hrafn/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(hrafn_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: hrafn_dir.join(".secret_key"),
            enabled,
        }
    }
