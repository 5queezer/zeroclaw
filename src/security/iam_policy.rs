...
/// Maps a single Nevis role to Hrafn permissions.
...
/// IAM policy engine that maps Nevis roles to Hrafn tool permissions.
///
...
mod tests {
    ...
    fn test_mappings() -> Vec<RoleMapping> {
        vec![
            ...
            RoleMapping {
                ...
                hrafn_permissions: vec!["file_read".into(), "memory_search".into()],
    ...
    fn duplicate_normalized_roles_are_rejected() {
        let mappings = vec![
            ...
            RoleMapping {
                nevis_role: " ADMIN ".into(),
                hrafn_permissions: vec!["file_read".into()],
