//! Flash Hrafn Arduino firmware via arduino-cli.
//!
...
/// Hrafn Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
...
/// Flash Hrafn firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("hrafn_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling Hrafn Arduino firmware...");
    ...
    println!("Hrafn firmware flashed successfully.");
