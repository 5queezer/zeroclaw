                ...
                for msg in msgs {
                    ...
                    if !self.is_number_allowed(&normalized_from) {
                        tracing::warn!(
                            ...
                            or run `hrafn onboard --channels-only` to configure interactively."
                        );
                        continue;
                    }
...
mod tests {
    ...
    fn whatsapp_parse_valid_text_message() {
        ...
        assert_eq!(msgs[0].content, "Hello Hrafn!");
    ...
    fn make_group_mention_channel() -> WhatsAppChannel {
        ...
        .with_group_mention_patterns(vec!["@?Hrafn".into()])
    }
    ...
    fn make_dm_mention_channel() -> WhatsAppChannel {
        ...
        .with_dm_mention_patterns(vec!["@?Hrafn".into()])
    }
    ...
    fn whatsapp_compile_skips_invalid_patterns() {
        let patterns =
            WhatsAppChannel::compile_mention_patterns(&["@?Hrafn".into(), "[invalid".into()]);
    ...
    fn whatsapp_strip_at_name() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?Hrafn".into()]);
        assert_eq!(
            WhatsAppChannel::strip_patterns(&pats, "@Hrafn what is the weather?"),
    ...
    fn whatsapp_strip_at_end() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?Hrafn".into()]);
        assert_eq!(
            WhatsAppChannel::strip_patterns(&pats, "Help me @Hrafn"),
            Some("Help me".into())
        );
    }
    ...
    fn whatsapp_strip_mid_sentence() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?Hrafn".into()]);
        assert_eq!(
            WhatsAppChannel::strip_patterns(&pats, "Hey @Hrafn how are you?"),
    ...
    fn whatsapp_strip_multiple_occurrences() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?Hrafn".into()]);
        assert_eq!(
            WhatsAppChannel::strip_patterns(&pats, "@Hrafn hello @Hrafn"),
            Some("hello".into())
        );
    }
    ...
    fn whatsapp_strip_collapses_whitespace() {
        let pats = WhatsAppChannel::compile_mention_patterns(&["@?Hrafn".into()]);
        assert_eq!(
            WhatsAppChannel::strip_patterns(&pats, "@Hrafn   status   please"),
    ...
    fn whatsapp_group_mention_accepts_and_strips_in_group() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [group_msg("111", "1", "@Hrafn what is the weather?")]
                    }
                }]
            }]
        });
    ...
    fn whatsapp_group_mention_strips_from_group_content() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [group_msg("111", "1", "Hey @Hrafn tell me a joke")]
                    }
                }]
            }]
        });
    ...
    fn whatsapp_group_mention_drops_mention_only_group_message() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [group_msg("111", "1", "@Hrafn")]
                    }
                }]
            }]
        });
    ...
    fn whatsapp_group_mention_case_insensitive_group_match() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [group_msg("111", "1", "@hrafn status")]
                    }
                }]
            }]
        });
    ...
    fn whatsapp_group_mention_dm_not_stripped() {
        ...
        let payload = serde_json::json!({
            "entry": [{
                "changes": [{
                    "value": {
                        "messages": [dm_msg("111", "1", "@Hrafn what is the weather?")]
                    }
                }]
            }]
        });
        ...
        assert_eq!(msgs.len(), 1);
        assert_eq!(
            msgs[0].content, "@Hrafn what is the weather?",
