...
impl RpiSystemContext {
    ...
    /// Write an `rpi0.md` hardware context file to `~/.hrafn/hardware/devices/`.
    ...
    pub fn write_hardware_context_file(&self) {
        ...
        let devices_dir = home.join(".hrafn").join("hardware").join("devices");
    ...
    fn device_profile_markdown(&self) -> String {
        ...
        let _ = writeln!(
            s,
            "- Runtime: Hrafn native (rppal — no serial, no mpremote)"
        );
        if let Some(ref iface) = self.wifi_interface {
            let _ = writeln!(s, "- WiFi interface: {}", iface);
        }
