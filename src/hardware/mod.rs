...
pub struct HardwareBootResult {
    ...
    /// Content of `~/.hrafn/hardware/` context files (HARDWARE.md, device
    /// profiles, and skills) for injection into the system prompt.
    pub context_files_prompt: String,
}

/// Load hardware context files from `~/.hrafn/hardware/` and return them
/// concatenated as a single markdown string ready for system-prompt injection.
///
/// Reads (if they exist):
/// 1. `~/.hrafn/hardware/HARDWARE.md`
/// 2. `~/.hrafn/hardware/devices/<alias>.md` for each discovered alias
/// 3. All `~/.hrafn/hardware/skills/*.md` files (sorted by name)
///
...
pub fn load_hardware_context_prompt(aliases: &[&str]) -> String {
    ...
    load_hardware_context_from_dir(&home.join(".hrafn").join("hardware"), aliases)
}
...
///
/// Without the feature: loads plugin tools from `~/.hrafn/tools/` only,
...
pub async fn boot(
    ...
    if uf2::find_rpi_rp2_mount().is_some() {
        ...
        tracing::info!("Say \"flash my pico\" to install Hrafn firmware automatically");
    }
...
/// Handle `hrafn hardware` subcommands.
...
fn run_info(chip: &str) -> Result<()> {
    ...
    #[cfg(not(feature = "probe"))]
    {
        ...
        println!();
        println!("Then run: hrafn hardware info --chip {}", chip);
        println!();
...
mod tests {
    ...
    fn hardware_context_contains_device_exec_rule() {
        ...
        if let Some(home) = directories::BaseDirs::new().map(|d| d.home_dir().to_path_buf()) {
            let hw_md = home.join(".hrafn").join("hardware").join("HARDWARE.md");
            if hw_md.exists() {
