...
//!
//! Wire protocol (Hrafn serial JSON):
//! ```text
...
/// Default baud rate for Hrafn serial devices.
...
/// Serial transport for Hrafn hardware devices.
///
...
impl HardwareSerialTransport {
    ...
    /// Attempt a ping handshake to verify Hrafn firmware is running.
    ///
    /// Opens the port, sends `{"cmd":"ping","params":{}}`, waits up to
    /// `PING_TIMEOUT_MS` for a response with `data.firmware == "hrafn"`.
    ///
    /// Returns `true` if a Hrafn device responds, `false` otherwise.
    ...
    pub async fn ping_handshake(&self) -> bool {
        ...
        match result {
            Ok(Ok(resp)) => {
                // Accept if firmware field is "hrafn" (in data or top-level)
                resp.ok
                    && resp
                        .data
                        .get("firmware")
