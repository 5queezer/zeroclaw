//! Wraps a discovered MCP tool as a hrafn [`Tool`] so it is dispatched
...
/// A hrafn [`Tool`] backed by an MCP server tool.
///
...
impl Tool for McpToolWrapper {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // Hrafn's security model injects `approved: bool` into built-in tool
...
mod tests {
    ...
    // Hrafn's security model injects `approved: bool` into built-in tool args.
