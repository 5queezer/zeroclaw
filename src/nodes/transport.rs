...
/// - `X-Hrafn-Timestamp` — unix epoch seconds
/// - `X-Hrafn-Nonce` — random UUID v4
/// - `X-Hrafn-Signature` — HMAC-SHA256 hex digest
///
...
impl NodeTransport {
    ...
    pub async fn send(
        ...
        let resp = self
            .http
            .post(&url)
            .header("X-Hrafn-Timestamp", timestamp.to_string())
            .header("X-Hrafn-Nonce", &nonce)
            .header("X-Hrafn-Signature", &signature)
