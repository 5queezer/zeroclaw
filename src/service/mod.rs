...
const SERVICE_LABEL: &str = "com.hrafn.daemon";
const WINDOWS_TASK_NAME: &str = "Hrafn Daemon";
...
/// Returns whether the Hrafn daemon service is currently running.
...
fn is_running_linux() -> bool {
    ...
    if run_capture(Command::new("systemctl").args(["--user", "is-active", "hrafn.service"]))
        .map(|out| out.trim() == "active")
        .unwrap_or(false)
    {
        return true;
    }
    run_capture(Command::new("rc-service").args(["hrafn", "status"]))
...
fn start_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            ...
            run_checked(Command::new("systemctl").args(["--user", "start", "hrafn.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["hrafn", "start"]))?;
        }
...
fn stop_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            let _ =
                run_checked(Command::new("systemctl").args(["--user", "stop", "hrafn.service"]));
        }
        InitSystem::Openrc => {
            let _ = run_checked(Command::new("rc-service").args(["hrafn", "stop"]));
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service stopped");
...
fn restart_linux(init_system: InitSystem) -> Result<()> {
    match init_system {
        InitSystem::Systemd => {
            run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]))?;
            run_checked(Command::new("systemctl").args(["--user", "restart", "hrafn.service"]))?;
        }
        InitSystem::Openrc => {
            run_checked(Command::new("rc-service").args(["hrafn", "restart"]))?;
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    println!("✅ Service restarted");
...
fn status_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let out = run_capture(Command::new("rc-service").args(["hrafn", "status"]))
                .unwrap_or_else(|_| "unknown".into());
            println!("Service state: {}", out.trim());
            println!("Unit: /etc/init.d/hrafn");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
fn logs_linux(config: &Config, init_system: InitSystem, lines: usize, follow: bool) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            // OpenRC logs go to /var/log/hrafn/error.log (as configured in the init script)
            let log_file = Path::new("/var/log/hrafn/error.log");
            if !log_file.exists() {
                // Fall back to access log
                let access_log = Path::new("/var/log/hrafn/access.log");
                if !access_log.exists() {
                    bail!("No log files found at /var/log/hrafn/. Is the service installed?");
                }
...
fn uninstall_linux(config: &Config, init_system: InitSystem) -> Result<()> {
    match init_system {
        ...
        InitSystem::Openrc => {
            let init_script = Path::new("/etc/init.d/hrafn");
            if init_script.exists() {
                if let Err(err) =
                    run_checked(Command::new("rc-update").args(["del", "hrafn", "default"]))
                {
                    eprintln!(
                        "⚠️  Warning: Could not remove hrafn from OpenRC default runlevel: {err}"
                    );
                }
                fs::remove_file(init_script)
                    .with_context(|| format!("Failed to remove {}", init_script.display()))?;
            }
            println!("✅ Service uninstalled (/etc/init.d/hrafn)");
        }
        InitSystem::Auto => unreachable!("Auto should be resolved before this point"),
    }
    Ok(())
...
/// corresponding `var/hrafn` directory.
///
...
fn detect_homebrew_var_dir(exe: &Path) -> Option<PathBuf> {
    ...
    // Symlinked binary: <prefix>/bin/hrafn
    // Cellar binary:    <prefix>/Cellar/hrafn/<version>/bin/hrafn
    let prefix = if path_str.contains("/Cellar/") {
        // Walk up from .../Cellar/hrafn/<ver>/bin/hrafn to the prefix
        ...
        while let Some(parent) = ancestor.parent() {
            ...
            if ancestor.file_name().map_or(false, |n| n == "Cellar") {
                ...
                return ancestor.parent().map(|p| p.join("var").join("hrafn"));
            }
        }
        return None;
    } else if let Some(bin_parent) = exe.parent() {
        // <prefix>/bin/hrafn → check if <prefix>/Cellar exists (Homebrew marker)
    ...
    prefix.map(|p| p.join("var").join("hrafn"))
}
...
fn install_macos(config: &Config) -> Result<()> {
    ...
    // data so that `brew services start hrafn` works out of the box.
    ...
    // When running under Homebrew, inject HRAFN_CONFIG_DIR and
...
fn install_linux_systemd(config: &Config) -> Result<()> {
    ...
    fs::write(&file, unit)?;
    let _ = run_checked(Command::new("systemctl").args(["--user", "daemon-reload"]));
    let _ = run_checked(Command::new("systemctl").args(["--user", "enable", "hrafn.service"]));
    ...
    if is_alpine {
        ...
        if !group_exists {
            let output = Command::new("addgroup")
                .args(["-S", "hrafn"])
                .output()
                .context("Failed to create hrafn group")?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                bail!("Failed to create hrafn group: {}", stderr.trim());
            }
            println!("✅ Created system group: hrafn");
        }
        ...
        let output = Command::new("adduser")
            ...
            .output()
            .context("Failed to create hrafn user")?;

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            bail!("Failed to create hrafn user: {}", stderr.trim());
        }
    } else {
        let output = Command::new("useradd")
            .args(["-r", "-s", "/sbin/nologin", "hrafn"])
            .output()
            .context("Failed to create hrafn user")?;
...
/// Change ownership of a path to hrafn:hrafn
#[cfg(unix)]
fn chown_to_hrafn(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["hrafn:hrafn", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to change ownership of {} to hrafn:hrafn: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_to_hrafn(_path: &Path) -> Result<()> {
    Ok(())
}

#[cfg(unix)]
fn chown_recursive_to_hrafn(path: &Path) -> Result<()> {
    let output = Command::new("chown")
        .args(["-R", "hrafn:hrafn", &path.to_string_lossy()])
        .output()
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        bail!(
            "Failed to recursively change ownership of {} to hrafn:hrafn: {}",
            path.display(),
            stderr.trim(),
        );
    }
...
#[cfg(not(unix))]
fn chown_recursive_to_hrafn(_path: &Path) -> Result<()> {
    Ok(())
}
    ...
    if let Some(user) = sudo_user {
        if let Ok(output) = Command::new("getent").args(["passwd", &user]).output() {
            if output.status.success() {
                ...
                if fields.len() >= 6 {
                    return Some(PathBuf::from(fields[5]).join(".hrafn"));
                }
            }
        }
    }
...
fn ensure_openrc_runtime_path_writable(path: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        ...
        bail!(
            "OpenRC runtime user 'hrafn' cannot write {} ({details}). \
             Re-run `sudo hrafn service install` and ensure ownership is hrafn:hrafn.",
            path.display(),
        );
    }
...
command_user="hrafn:hrafn"
pidfile="/run/${{RC_SVCNAME}}.pid"
umask 027
output_log="/var/log/hrafn/access.log"
error_log="/var/log/hrafn/error.log"
...
start_pre() {{
    checkpath --directory --owner hrafn:hrafn --mode 0750 /var/lib/hrafn
}}
"#,
...
fn resolve_openrc_executable() -> Result<PathBuf> {
    let preferred = Path::new("/usr/local/bin/hrafn");
    if preferred.exists() {
...
fn install_linux_openrc(config: &Config) -> Result<()> {
    if !is_root() {
        bail!(
            "OpenRC service installation requires root privileges.\n\
             Please run with sudo: sudo hrafn service install"
        );
    }
    ...
    let config_dir = Path::new("/etc/hrafn");
    let workspace_dir = config_dir.join("workspace");
    let log_dir = Path::new("/var/log/hrafn");
    ...
    if !workspace_dir.exists() {
        ...
        chown_to_hrafn(&workspace_dir)?;
        println!(
            "✅ Created directory: {} (owned by hrafn:hrafn)",
    ...
    chown_recursive_to_hrafn(config_dir)?;
    ...
    let init_path = Path::new("/etc/init.d/hrafn");
...
fn install_windows(config: &Config) -> Result<()> {
    ...
    let wrapper = logs_dir.join("hrafn-daemon.cmd");
    let stdout_log = logs_dir.join("daemon.stdout.log");
    let stderr_log = logs_dir.join("daemon.stderr.log");
...
mod tests {
    ...
    fn linux_service_file_has_expected_suffix() {
        ...
        assert!(path.ends_with(".config/systemd/user/hrafn.service"));
    }

    #[test]
    fn windows_task_name_is_constant() {
        assert_eq!(windows_task_name(), "Hrafn Daemon");
    }
    ...
    fn generate_openrc_script_contains_required_directives() {
        ...
        let exe_path = PathBuf::from("/usr/local/bin/hrafn");
        let script = generate_openrc_script(&exe_path, Path::new("/etc/hrafn"));

        assert!(script.starts_with("#!/sbin/openrc-run"));
        assert!(script.contains("name=\"hrafn\""));
        assert!(script.contains("description=\"Hrafn daemon\""));
        assert!(script.contains("command=\"/usr/local/bin/hrafn\""));
        assert!(script.contains("command_args=\"--config-dir /etc/hrafn daemon\""));
        assert!(!script.contains("env HRAFN_CONFIG_DIR"));
        assert!(!script.contains("env HRAFN_WORKSPACE"));
        assert!(script.contains("command_background=\"yes\""));
        assert!(script.contains("command_user=\"hrafn:hrafn\""));
        assert!(script.contains("pidfile=\"/run/${RC_SVCNAME}.pid\""));
        assert!(script.contains("umask 027"));
        assert!(script.contains("output_log=\"/var/log/hrafn/access.log\""));
        assert!(script.contains("error_log=\"/var/log/hrafn/error.log\""));
    ...
    fn generate_openrc_script_sets_home_for_browser() {
        ...
        assert!(
            script.contains("export HOME=\"/var/lib/hrafn\""),
    ...
    fn generate_openrc_script_creates_home_directory() {
        ...
        assert!(
            script.contains("checkpath --directory --owner hrafn:hrafn"),
            "start_pre must ensure /var/lib/hrafn exists with correct ownership"
        );
    }
    ...
    fn systemd_unit_contains_home_and_pass_environment() {
        let unit = "[Unit]\n\
             ...
             [Service]\n\
             Type=simple\n\
             ExecStart=/usr/local/bin/hrafn daemon\n\
             Restart=always\n\
             RestartSec=3\n\
             # Ensure HOME is set so headless browsers can create profile/cache dirs.\n\
             Environment=HOME=%h\n\
    ...
    fn warn_if_binary_in_home_detects_home_path() {
        ...
        let home_path = PathBuf::from("/home/user/.cargo/bin/hrafn");
        assert!(home_path.to_string_lossy().contains("/home/"));
        assert!(home_path.to_string_lossy().contains(".cargo/bin"));

        let cargo_path = PathBuf::from("/home/user/.cargo/bin/hrafn");
        assert!(cargo_path.to_string_lossy().contains(".cargo/bin"));

        let system_path = PathBuf::from("/usr/local/bin/hrafn");
    ...
    fn detect_homebrew_var_dir_from_cellar_path() {
        let exe = PathBuf::from("/opt/homebrew/Cellar/hrafn/1.2.3/bin/hrafn");
        let var_dir = detect_homebrew_var_dir(&exe);
        assert_eq!(var_dir, Some(PathBuf::from("/opt/homebrew/var/hrafn")));
    }

    #[test]
    fn detect_homebrew_var_dir_intel_cellar_path() {
        let exe = PathBuf::from("/usr/local/Cellar/hrafn/1.0.0/bin/hrafn");
        let var_dir = detect_homebrew_var_dir(&exe);
        assert_eq!(var_dir, Some(PathBuf::from("/usr/local/var/hrafn")));
    }

    #[test]
    fn detect_homebrew_var_dir_non_homebrew_path() {
        let exe = PathBuf::from("/home/user/.cargo/bin/hrafn");
        let var_dir = detect_homebrew_var_dir(&exe);
    ...
    fn openrc_writability_probe_falls_back_to_su() {
        let (program, args) =
            build_openrc_writability_probe_command(Path::new("/etc/hrafn/workspace"), false);
        ...
        assert_eq!(
            ...
            vec![
                ...
                "/bin/sh".to_string(),
                "-c".to_string(),
                "test -w '/etc/hrafn/workspace'".to_string(),
                "hrafn".to_string()
            ]
        );
    }
    ...
    fn tail_file_errors_on_missing_file() {
        let missing = Path::new("/tmp/hrafn-test-nonexistent-log-file.log");
