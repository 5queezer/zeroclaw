# Hrafn Development Environment

A fully containerized development sandbox for Hrafn agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`hrafn-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `hrafn` CLI commands manually, debug the binary, or check logs internally.
...
The agent configuration lives in `target/.hrafn` (mounted to `/hrafn-data/.hrafn`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.hrafn` (config/DB) but leaves the `playground/` directory intact. To fully wipe workspace data, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `hrafn-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=hrafn-target`) to speed repeat local image builds.
