...
ASSET_URL="https://github.com/5queezer/hrafn/releases/download/${TAG}/${ASSET_NAME}"
TEMP_DIR="$(mktemp -d -t hrafn-termux-test-XXXXXX)"
...
# --- Test 5: Contains hrafn binary ---
info "Checking archive contents"
CONTENTS=$(tar -tzf "$TEMP_DIR/$ASSET_NAME")
if echo "$CONTENTS" | grep -q "^hrafn$"; then
  pass "Archive contains 'hrafn' binary"
else
  fail "Archive does not contain 'hrafn' binary"
  echo "Contents:"
  echo "$CONTENTS" | sed 's/^/  /'
fi
...
CHECKSUMS_URL="https://github.com/5queezer/hrafn/releases/download/${TAG}/SHA256SUMS"
