...
build_smoke_image() {
  if docker buildx version >/dev/null 2>&1; then
    ...
    DOCKER_BUILDKIT=1 docker build --target dev -t hrafn-local-smoke:latest .
  fi
}

print_help() {
  cat <<'EOF'
Hrafn Local CI in Docker
...
case "$1" in
  ...
  docker-smoke)
    build_smoke_image
    docker run --rm hrafn-local-smoke:latest --version
    ;;
