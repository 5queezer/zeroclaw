...
//!
//! Requires:
//! - Live Gemini OAuth profile in `~/.hrafn/auth-profiles.json` with refresh_token
...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    // Find ~/.hrafn/auth-profiles.json
    let home = env::var("HOME").expect("HOME env var not set");
    let hrafn_dir = PathBuf::from(home).join(".hrafn");
    let auth_profiles_path = hrafn_dir.join("auth-profiles.json");
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: hrafn auth login --provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No Gemini OAuth profile found. Run: hrafn auth login --provider gemini"
            )
        })?
        .clone();
    ...
    // This will load auth from ~/.hrafn/auth-profiles.json (with expired token)
    let provider = hrafn::providers::create_provider("gemini", None)?;
