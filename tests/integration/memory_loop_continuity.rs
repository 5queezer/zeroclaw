...
use hrafn::memory::sqlite::SqliteMemory;
use hrafn::memory::traits::{Memory, MemoryCategory};
use hrafn::providers::ToolCall;
...
async fn compressor_with_memory_saves_summary() {
    use hrafn::agent::context_compressor::{ContextCompressionConfig, ContextCompressor};
    use hrafn::providers::traits::ChatMessage;
...
async fn agent_survives_large_tool_output() {
    use hrafn::tools::{Tool, ToolResult};
...
async fn consolidation_extracts_facts_to_memory() {
    ...
    let result = hrafn::memory::consolidation::consolidate_turn(
        &provider,
        "test-model",
        mem.as_ref(),
...
async fn memory_survives_rapid_consolidation() {
    ...
    for i in 0..10 {
        ...
        let _ = hrafn::memory::consolidation::consolidate_turn(
            &provider,
            "test-model",
            mem.as_ref(),
...
async fn session_backend_persists_messages() {
    use hrafn::channels::session_backend::SessionBackend;
    use hrafn::channels::session_sqlite::SqliteSessionBackend;
    use hrafn::providers::traits::ChatMessage;
