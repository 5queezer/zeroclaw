...
This guide covers testing the Telegram channel integration for Hrafn.
...
Solution: Check user allowlist
  ...
  4. Run: hrafn onboard --channels-only
```
...
| ---------------------- | ---------- | -------------------------------- |
| Health check time      | <5s        | `time hrafn channel doctor`   |
| First response time    | <3s        | Time from sending to receiving   |
| Message split overhead | <50ms      | Check logs for timing            |
| Memory usage           | <10MB      | `ps aux \| grep hrafn`        |
| Binary size            | ~3-4MB     | `ls -lh target/release/hrafn` |
...
```bash
# Maximum logging
RUST_LOG=trace hrafn channel start
...
# Validate config
cat ~/.hrafn/config.toml | grep -A 3 "\[channels_config.telegram\]"
```
...
- [Hrafn Main README](../../README.md)
- [Contributing Guide](../../CONTRIBUTING.md)
- [Issue Tracker](https://github.com/5queezer/hrafn/issues)
