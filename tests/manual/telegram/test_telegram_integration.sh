#!/bin/bash
# Hrafn Telegram Integration Test Suite
...
if [ -f "target/release/hrafn" ]; then
    BINARY_SIZE=$(ls -lh target/release/hrafn | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/hrafn 2>/dev/null || stat -c%s target/release/hrafn)
...
CONFIG_PATH="$HOME/.hrafn/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'hrafn onboard' first"
    fi
else
    warn "No config file found - run 'hrafn onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/hrafn channel doctor 2>&1 || true)
END_TIME=$(date +%s)
