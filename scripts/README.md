...
This directory contains everything needed to cross-compile Hrafn and deploy it to a Raspberry Pi over SSH.
...
| `rpi-config.toml` | Production config template deployed to `~/.hrafn/config.toml` |
| `hrafn.service` | systemd unit file installed on the Pi |
...
| `RPI_DIR` | `~/hrafn` | Remote directory for the binary and `.env` |
...
2. **Stop service** — runs `sudo systemctl stop hrafn` on the Pi (continues if not yet installed).
3. **Create remote directory** — ensures `$RPI_DIR` exists on the Pi.
4. **Copy binary** — SCPs the compiled binary to `$RPI_DIR/hrafn`.
...
6. **Deploy config** — copies `rpi-config.toml` to `~/.hrafn/config.toml`, preserving any `api_key` already present in the file.
7. **Install systemd service** — copies `hrafn.service` to `/etc/systemd/system/`, then enables and restarts it.
...
## Interacting with Hrafn on the Pi
...
```bash
ssh pi@raspberrypi.local 'journalctl -u hrafn -f'
```
...
Hrafn is deployed with the `peripheral-rpi` feature, which enables two LLM-callable tools:
...
|------------|--------|-------------|
| `~/hrafn/hrafn` | compiled binary | Main agent binary |
| `~/hrafn/.env` | created on first deploy | API key and environment variables |
| `~/.hrafn/config.toml` | `rpi-config.toml` | Agent configuration |
| `/etc/systemd/system/hrafn.service` | `hrafn.service` | systemd service unit |
...
To customise, edit `~/.hrafn/config.toml` directly on the Pi and restart the service.
...
```bash
ssh pi@raspberrypi.local 'sudo systemctl status hrafn'
ssh pi@raspberrypi.local 'journalctl -u hrafn -n 50 --no-pager'
```
...
```bash
ssh pi@raspberrypi.local 'file ~/hrafn/hrafn'
