#!/usr/bin/env bash
# deploy-rpi.sh — cross-compile Hrafn for Raspberry Pi and deploy via SSH.
#
...
#   RPI_DIR         — remote deployment dir           (default: /home/$RPI_USER/hrafn)
...
RPI_DIR="${RPI_DIR:-/home/${RPI_USER}/hrafn}"
TARGET="aarch64-unknown-linux-gnu"
FEATURES="hardware,peripheral-rpi"
BINARY="target/${TARGET}/release/hrafn"
...
echo "==> Building Hrafn for Raspberry Pi (${TARGET})"
...
echo ""
echo "==> Stopping hrafn service (if running)"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" \
  "sudo systemctl stop hrafn 2>/dev/null || true"
...
# ── 4. Deploy binary ──────────────────────────────────────────────────────────
echo ""
echo "==> Deploying binary to ${RPI_USER}@${RPI_HOST}:${RPI_DIR}/hrafn"
${SCP_CMD} ${SCP_OPTS} "${BINARY}" "${RPI_USER}@${RPI_HOST}:${RPI_DIR}/hrafn"
...
# ── 5. Deploy config ─────────────────────────────────────────────────────────
CONFIG_DEST="/home/${RPI_USER}/.hrafn/config.toml"
echo ""
echo "==> Deploying config to ${CONFIG_DEST}"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" "mkdir -p /home/${RPI_USER}/.hrafn"
...
SERVICE_DEST="/etc/systemd/system/hrafn.service"
echo ""
echo "==> Installing systemd service (requires sudo on the Pi)"
${SCP_CMD} ${SCP_OPTS} "scripts/hrafn.service" "${RPI_USER}@${RPI_HOST}:/tmp/hrafn.service"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" \
  "sudo mv /tmp/hrafn.service ${SERVICE_DEST} && \
   sudo systemctl daemon-reload && \
   sudo systemctl enable hrafn && \
   sudo systemctl restart hrafn && \
   sudo systemctl status hrafn --no-pager || true"
...
# ── 8. Reset ACT LED trigger so Hrafn can control it ──────────────────────
echo ""
...
echo ""
echo "    Hrafn is running at http://${RPI_HOST}:8080"
echo "    POST /api/chat  — chat with the agent"
echo "    GET  /health    — health check"
echo ""
echo "    To check logs: ssh ${RPI_USER}@${RPI_HOST} 'journalctl -u hrafn -f'"
