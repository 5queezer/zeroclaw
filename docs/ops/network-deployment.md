# Network Deployment — Hrafn on Raspberry Pi and Local Network

This document covers deploying Hrafn on a Raspberry Pi or other host on your local network, with Telegram and optional webhook channels.
...
|------|----------------------|----------|
| **Telegram polling** | No | Hrafn polls Telegram API; works from anywhere |
| **Matrix sync (including E2EE)** | No | Hrafn syncs via Matrix client API; no inbound webhook required |
...
**Key:** Telegram, Discord, Slack, and Nostr use **outbound connections** — Hrafn connects to external servers/relays. No port forwarding or public IP required.

---

## 2. Hrafn on Raspberry Pi
...
- Telegram channel works: Hrafn polls Telegram API (outbound)
...
2. Start a tunnel:
   ...
   ```
   Or use `hrafn tunnel` (see tunnel docs).

3. Hrafn will refuse `0.0.0.0` unless `allow_public_bind = true` or a tunnel is active.
...
- Hrafn calls `https://api.telegram.org/bot{token}/getUpdates`
...
Run `hrafn daemon` — Telegram channel starts automatically.
...
- Keep one runtime instance for the same token (recommended: `hrafn daemon` service).
...
- [ ] Run `hrafn daemon --host 127.0.0.1 --port 42617` (Telegram works without 0.0.0.0)
...
Hrafn supports OpenRC for Alpine Linux and other distributions using the OpenRC init system. OpenRC services run **system-wide** and require root/sudo.

### 7.1 Prerequisites

- Alpine Linux (or another OpenRC-based distro)
- Root or sudo access
- A dedicated `hrafn` system user (created during install)
...
This creates:
- Init script: `/etc/init.d/hrafn`
- Config directory: `/etc/hrafn/`
- Log directory: `/var/log/hrafn/`

### 7.3 Configuration

Manual config copy is usually not required.

`sudo hrafn service install` automatically prepares `/etc/hrafn`, migrates existing runtime state from your user setup when available, and sets ownership/permissions for the `hrafn` service user.

If no prior runtime state is available to migrate, create `/etc/hrafn/config.toml` before starting the service.
...
| Command | Description |
|---------|-------------|
| `sudo rc-service hrafn start` | Start the daemon |
| `sudo rc-service hrafn stop` | Stop the daemon |
| `sudo rc-service hrafn status` | Check service status |
| `sudo rc-service hrafn restart` | Restart the daemon |
| `sudo hrafn service status` | Hrafn status wrapper (uses `/etc/hrafn` config) |
...
| Log | Path |
|-----|------|
| Access/stdout | `/var/log/hrafn/access.log` |
| Errors/stderr | `/var/log/hrafn/error.log` |
...
- The service runs as the `hrafn:hrafn` user (least privilege)
- Config must be at `/etc/hrafn/config.toml` (explicit path in init script)
- If the `hrafn` user does not exist, install will fail with instructions to create it

### 7.9 Checklist: Alpine/OpenRC Deployment

- [ ] Install: `sudo hrafn service install`
- [ ] Enable: `sudo rc-update add hrafn default`
- [ ] Start: `sudo rc-service hrafn start`
- [ ] Verify: `sudo rc-service hrafn status`
- [ ] Check logs: `/var/log/hrafn/error.log`
