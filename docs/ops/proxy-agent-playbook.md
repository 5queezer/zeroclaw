...
- **Audience:** operators and maintainers running Hrafn in proxied networks.
- **Scope:** `proxy_config` actions, mode selection, verification flow, and troubleshooting.
- **Non-goals:** generic network debugging outside Hrafn runtime behavior.
...
### 1.1 Proxy only Hrafn internal traffic
...
- [Section 4](#4-mode-a--proxy-only-for-hrafn-internals)
...
|---|---|---|---|
| `hrafn` | Hrafn internal HTTP clients | No | Normal runtime proxying without process-level side effects |
...
## 4. Mode A — Proxy Only for Hrafn Internals

Use when Hrafn provider/channel/tool HTTP traffic should use proxy, without exporting process-level proxy env vars.
...
- Runtime proxy is active for Hrafn HTTP clients.
