...
- `cargo check` / `cargo build` appears stuck at `Checking hrafn` for a long time
- repeated `Blocking waiting for file lock on package cache` or `build directory`

Why this happens in Hrafn:
...
### `hrafn` command not found after install

Symptom:

- install succeeds but shell cannot find `hrafn`
...
- stop extra `hrafn daemon` / `hrafn channel start` processes
