...
The rest of Hrafn speaks a single language: `ZcCommand` → `ZcResponse`.
...
DatasheetTool.call(args)
  ...
  → "download": fetch PDF from args["url"] → save to ~/.hrafn/hardware/datasheets/
...
```
 SDK FILES          aardvark-sys            Hrafn core
 (vendor/)          (crates/)               (src/)
─────────────────────────────────────────────────────────────────
