...
This page defines the fastest supported path to install and initialize Hrafn.
...
Default behavior is **app-only** (build/install Hrafn) and expects existing Rust toolchain.
...
This builds a local Hrafn image and launches onboarding inside a container while
persisting config/workspace to `./.hrafn-docker`.

Container CLI defaults to `docker`. If Docker CLI is unavailable and `podman` exists,
the installer auto-falls back to `podman`. You can also set `HRAFN_CONTAINER_CLI`
explicitly (for example: `HRAFN_CONTAINER_CLI=podman ./install.sh --docker`).
...
Docker tag (`HRAFN_DOCKER_IMAGE`, default: `hrafn-bootstrap:local`); if missing,
it pulls `ghcr.io/5queezer/hrafn:latest` and tags it locally before running.
...
are persisted in the data directory (default: `./.hrafn-docker`, or `~/.hrafn-docker`
when bootstrapping via `curl | bash`). You can override this path with `HRAFN_DOCKER_DATA_DIR`.
...
The simplest way to run Hrafn long-term in Docker/Podman is with the provided
`docker-compose.yml` at the repository root. It uses a named volume (`hrafn-data`)
...
If you installed via `./install.sh --docker` and want to reuse the `.hrafn-docker`
...
podman run -d --name hrafn \
  --restart unless-stopped \
  --userns keep-id \
  --user "$(id -u):$(id -g)" \
  -v "$PWD/.hrafn-docker/.hrafn:/hrafn-data/.hrafn:Z" \
  -v "$PWD/.hrafn-docker/workspace:/hrafn-data/workspace:Z" \
  -e HOME=/hrafn-data \
  -e HRAFN_WORKSPACE=/hrafn-data/workspace \
  -p 42617:42617 \
  hrafn-bootstrap:local \
...
# Remove the container (data in volumes/.hrafn-docker is preserved)
docker rm hrafn
...
saved in `.hrafn-docker/.hrafn/config.toml` and do not need to be passed again.
...
- `--install-system-deps`
- `--install-rust`
- `--skip-build` (in `--docker` mode: use local image if present, otherwise pull `ghcr.io/5queezer/hrafn:latest`)
- `--skip-install`
- `--provider <id>`
