...
This guide covers native Nextcloud Talk integration for Hrafn.
...
Add this section in `~/.hrafn/config.toml`:
...
allowed_users = ["*"]
# bot_name is the Nextcloud Talk display name of the bot (e.g. "hrafn").
...
When `webhook_secret` is configured, Hrafn verifies:
...
- Hrafn ignores bot-originated webhook events (`actorType = bots`).
- Hrafn ignores non-message/system events.
...
3. Confirm Hrafn receives and replies in the same room.
