...
This document is the canonical reference for channel configuration in Hrafn.
...
6. **Stale runtime state**: config changed but `hrafn daemon` was not restarted.

---

## 1. Configuration Namespace

All channel settings live under `channels_config` in `~/.hrafn/config.toml`.
...
When running `hrafn channel start` (or daemon mode), Telegram and Discord now support sender-scoped runtime switching:
...
- Model cache previews come from `hrafn models refresh --provider <ID>`.
- These are runtime chat commands, not CLI subcommands.

## Inbound Image Marker Protocol

Hrafn supports multimodal input through inline message markers:
...
If `[channels_config.matrix]`, `[channels_config.lark]`, or `[channels_config.feishu]` is present but the corresponding feature is not compiled in, `hrafn channel list`, `hrafn channel doctor`, and `hrafn channel start` will report that the channel is intentionally skipped for this build.
...
access_token = "syt_..."
user_id = "@hrafn:matrix.example.com"   # optional, recommended for E2EE
...
Hrafn supports two WhatsApp backends:
...
allowed_users = ["*"]
# bot_name = "hrafn"  # display name of the bot; filters own messages to prevent feedback loops
```
...
- `HRAFN_NEXTCLOUD_TALK_WEBHOOK_SECRET` overrides config secret.
...
- `HRAFN_LINQ_SIGNING_SECRET` overrides config secret.
...
5. Restart `hrafn daemon` after config changes.
