...
3. Generic fallback env vars: `HRAFN_API_KEY` then `API_KEY`
...
Hrafn supports automatic failover to alternative providers when the primary encounters:
...
When a provider returns 429 (rate limit), Hrafn:
...
- After multimodal normalization, Hrafn sends image payloads through Ollama's native `messages[].images` field.
- If a non-vision provider is selected, Hrafn returns a structured capability error instead of silently ignoring images.
...
- Hrafn normalizes a trailing `/api` in `api_url` automatically.
...
- Model discovery: `hrafn models refresh --provider llamacpp`
...
- Model discovery: `hrafn models refresh --provider sglang`
...
- Model discovery: `hrafn models refresh --provider vllm`
...
- Model discovery: `hrafn models refresh --provider osaurus`
...
- Model discovery: `hrafn models refresh --provider nvidia`
