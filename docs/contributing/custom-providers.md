...
Hrafn supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
# or: export HRAFN_API_KEY="your-api-key"
hrafn agent
```

## llama.cpp Server (Recommended Local Setup)

Hrafn includes a first-class local provider for `llama-server`:
...
You do not need to export `HRAFN_API_KEY=dummy` for this flow.

## SGLang Server

Hrafn includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
Hrafn includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
