# Adding Boards and Tools — Hrafn Hardware Guide

This guide explains how to add new hardware boards and custom tools to Hrafn.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.hrafn/config.toml)
hrafn peripheral add nucleo-f401re /dev/ttyACM0
hrafn peripheral add arduino-uno /dev/cu.usbmodem12345
hrafn peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, Hrafn can index PDF files:
...
2. **Add to config** — `hrafn peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `hrafn peripheral list` | List configured boards |
| `hrafn peripheral add <board> <path>` | Add board (writes config) |
| `hrafn peripheral flash` | Flash Arduino firmware |
| `hrafn peripheral flash-nucleo` | Flash Nucleo firmware |
| `hrafn hardware discover` | List USB devices |
| `hrafn hardware info` | Chip info via probe-rs |
