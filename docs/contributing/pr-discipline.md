...
Rules for pull request quality, attribution, privacy, and handoff in Hrafn.
...
- If identity-like context is unavoidable, use Hrafn-scoped roles/labels only (e.g., `HrafnAgent`, `HrafnOperator`, `hrafn_user`).
- Recommended identity-safe naming palette:
    - actor labels: `HrafnAgent`, `HrafnOperator`, `HrafnMaintainer`, `hrafn_user`
    - service/runtime labels: `hrafn_bot`, `hrafn_service`, `hrafn_runtime`, `hrafn_node`
    - environment labels: `hrafn_project`, `hrafn_workspace`, `hrafn_channel`
