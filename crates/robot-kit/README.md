...
A complete toolkit for building AI-powered robots with Hrafn. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build Hrafn with robot tools

```bash
# Clone and build
git clone https://github.com/5queezer/hrafn
cd hrafn
cargo build -p hrafn-robot-kit --release
```
...
# Copy config
mkdir -p ~/.hrafn
cp crates/robot-kit/robot.toml ~/.hrafn/
cp crates/robot-kit/SOUL.md ~/.hrafn/workspace/
...
# Test in mock mode
./target/release/hrafn agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/hrafn agent -m "Move forward 1 meter"
```
...
```rust
use hrafn_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `hrafn`, add a thin adapter that maps this
...
mkdir hrafn-robot-kit
cp -r target/release/hrafn hrafn-robot-kit/
cp -r examples/robot_kit hrafn-robot-kit/
cp -r ~/.hrafn hrafn-robot-kit/dot-hrafn

# Include models
mkdir -p hrafn-robot-kit/models
cp ~/.hrafn/models/ggml-base.bin hrafn-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf hrafn-robot-kit.tar.gz hrafn-robot-kit/

# Copy to USB
cp hrafn-robot-kit.tar.gz /media/usb/TarBalls/
```
